...
namespace MeghalayaUIP.Dept.PreReg
{
    public partial class PreRegApplDeptProcess : System.Web.UI.Page
    {
        ...
        public void BindaApplicatinDetails()
        {
            try
            {
                ...
                if (Session["UNITID"] != null && Session["INVESTERID"] != null)
                {
                    ...
                    prd.Role = Convert.ToInt32(ObjUserInfo.PreRegRoleid == null ? ObjUserInfo.Roleid : ObjUserInfo.PreRegRoleid); //Convert.ToInt32(ObjUserInfo.Roleid);
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        //}
                        if (Request.QueryString["status"].ToString() == "TOBEPROCESSED" && (Convert.ToString(ds.Tables[6].Rows[0]["PRDA_STAGEID"]) == "6" || Convert.ToString(ds.Tables[6].Rows[0]["PRDA_STAGEID"]) == "13"))
                        {
                        ...
                        {
                            if (ds.Tables.Count > 6 && ds.Tables[6].Rows.Count > 0 && ds.Tables[6].Rows[0]["PRDA_STAGEID"] != DBNull.Value)
                            {
                                if (Convert.ToString(ds.Tables[6].Rows[0]["PRDA_STAGEID"]) == "18")
                                {
                                    siteplan.Visible = true;
                                }
                                else
                                {
        ...
        protected void btnDICProcess_Click(object sender, EventArgs e)
        {
            try
            {
