...
{
    public partial class PDCLDetails : System.Web.UI.Page
    {
        MasterBAL mstrBAL = new MasterBAL();
        SVRCBAL objSrvcbal = new SVRCBAL();
        string ErrorMsg = "", UnitID, result, Questionnaire;
        ...
        protected void GetAppliedorNot()
        {
            try
            {
                ...
                ds = objSrvcbal.GetsrvcapprovalID(hdnUserID.Value, Convert.ToString(Session["SRVCQID"]), "14", "84");
                ...
                else
                {
                    if (Request.QueryString.Count > 0)
                    {
                        if (Convert.ToString(Request.QueryString[0]) == "N")
                            Response.Redirect("~/User/Services/EWasteDetails.aspx?Next=" + "N");
        ...
        public void BindData()
        {
            ...
            {
                DataSet ds = new DataSet();
                ds = objSrvcbal.GetSrvcPDCLDetails(hdnUserID.Value, Convert.ToString(Session["SRVCQID"]));
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ErrorMsg == "")
                {
                 ...
                 //   Power.UnitId = Convert.ToString(Session["SRVCUNITID"]);
                    Power.Createdby = hdnUserID.Value;
                    Power.Questionnariid = Convert.ToString(Session["SRVCQID"]);
            ...
            try
            {
                ...
                if (fupduly.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "Agreement from duly filled" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
            ...
            try
            {
                ...
                if (fupownership.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "Proof of ownership (in case the applicant is owner)" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
            ...
            try
            {
                ...
                if (fuppole.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "Number of the nearest electic pole" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
            ...
            try
            {
                ...
                if (fupowner.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "No objection certificate of the owner of the premises" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
            ...
            try
            {
                ...
                if (fupPCB.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "Pollution Control Board-If applicable" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
            ...
            try
            {
                ...
                if (fupBuilding.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "Building permission from the concerned authority" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
            ...
            try
            {
                ...
                if (fupOccupancy.HasFile)
                {
                    ...
                    if (Error == "")
                    {
                        string sFileDir = ConfigurationManager.AppSettings["SRVCAttachments"];
                        string serverpath = sFileDir + hdnUserID.Value + "\\"
                        + Convert.ToString(Session["SRVCQID"]) + "\\" + "Occupancy Certificate from MUDA for areas within shillong Municipal limits" + "\\";
                        if (!Directory.Exists(serverpath))
                        {
                            Directory.CreateDirectory(serverpath);
                        }
